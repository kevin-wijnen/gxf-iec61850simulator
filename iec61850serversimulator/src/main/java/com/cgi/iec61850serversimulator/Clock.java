...
public class Clock {
    ...
    String beginDateDST; // time zone abbreviation, in format: M<month in
    ...
    String endDateDST; // time zone abbreviation, in format: M<month in
                       // MM>.<which 'th' day in the month, 5 = 5th/last day of
                       // the month>.<which type of day it should switch? 0 =
                       // Sunday?>/<which hour it switches on>
    short deviationDST; // in minutes, how many minutes will be switched over
                        // when DST is enabled/disabled (Standard = 60 minutes =
                        // 1 hour)
    boolean enableDST;
    boolean enableNTP;
    String ipAddressNTP; // if multiple are in use, use ; to separate them
    int timeSyncInterval; // in minutes, regarding synchronizing time with the
                          // NTP server
